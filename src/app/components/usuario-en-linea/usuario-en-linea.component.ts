...
export class UsuarioEnLineaComponent implements OnInit {
  ...
  selectedUsuarioText: string = "Seleccionar colaborador";
  ...
  listarUsuariosEnLinea() {
    ...
    this.usuarioEnLineaService.getUsuariosEnLinea(this.selectedLineaObject.id, this.selectedCalibradorObject.id, this.rutBusqueda, this.desde, this.hasta).subscribe(
      res => {
        ...
        //almaceno en una variable auxiliar toda la lista inicial de usurios en linea
        if (this.selectedLineaObject.id && this.selectedCalibradorObject.id && this.rutBusqueda == null && this.desde && this.hasta == null) {
          this.usuariosEnLineaAux = this.usuariosEnLinea;
        }
        ...
        for (let element of this.usuariosEnLinea) {
          ...
          this.timeFinish = element.fecha_termino;
          this.timeFinish = this.timeFinish.substring(11, 19);
          this.dateFinish = element.fecha_termino;
          this.dateFinish = this.dateFinish.substring(0, 10);
  ...
  changeSelectedCalibrador(newSelected: any) {
    ...
    this.selectedLineaText="Seleccionar linea";
  ...
  agregarUsuarioEnLinea(modalAsignarUsuario, modalUsuarioExisteEnLinea ) {
    ...
    for (let i = 0; i < this.usuariosEnLineaAux.length; i++) {
      if (this.usuariosEnLineaAux[i].usuario_rut == this.selectedUsuarioObject.rut && this.usuariosEnLineaAux[i].fecha_termino == "") {
        if(this.usuariosEnLineaAux[i].id_linea==this.selectedLineaObject.id){
          return this.open(modalUsuarioExisteEnLinea);
        }
        this.currentUsuarioEnLineaSelected=this.usuariosEnLineaAux[i];
        return this.open(modalAsignarUsuario);
      }
    }
    this.asignarUsuarioALinea();
    this.selectedUsuarioObject=null;
    this.selectedCalibradorText="Seleccionar colaborador"
  }

  asignarUsuarioALinea() {
  ...
  cerrarTurnoYasignarUsuarioALinea() {    
    this.currentUsuarioEnLineaSelected.fecha_termino = this.fecha();
    this.usuarioEnLineaService.cerrarTurnoUsuarioEnLinea(this.currentUsuarioEnLineaSelected.id, this.currentUsuarioEnLineaSelected).subscribe(
      res => {
        let usuarioEnLinea = new UsuarioEnLinea(null, this.selectedLineaObject.id, this.selectedLineaObject.nombre, 0, "", "", this.selectedUsuarioObject.id, this.selectedUsuarioObject.rut, this.selectedUsuarioObject.nombre, this.selectedUsuarioObject.apellido, this.selectedUsuarioObject.rfid, this.dateSave, "", this.selectedCalibradorObject.id, this.selectedCalibradorObject.nombre);
        console.log(usuarioEnLinea);
        this.usuarioEnLineaService.saveUsuarioEnLinea(usuarioEnLinea).subscribe(
      ...
      err=>{
        this.toastr.error('No se pudo cambiar de linea a usuario', 'Oops');
      }
    )
  }
